...
NOTICE: These tools cannot prevent tracking done by websites that you are logged into. For example, if you are logged into google account and watching youtube videos, that activity is linked to your account.
...
* The project aims to protect users from these hidden background tracking mechanisms and the "filter bubbles" that most don't know exists. While using Privacy Fighter (this setup) every new tab is a completely new session, if you don't log in on a website you should see the results without any personalization thus escaping the filter bubbles. (It can't protect your identity or privacy when you use a service (i.g Youtube/Facebook) while you are logged in.)
* **Everyone is uniquely identifiable on the web**, even if you use privacy protecting extensions/configurations. (see section: []). The project aims to create a single configuration set that minimises entropy (uniquely identifiable information). When used by many and would make us all less fingerprintable. **This is the only way to combat fingerprinting.**
...
* The goal is not to blindly gather extensions (addons) or disable as many browser functionalities (using firefox preferences) as possible. Neither it is to spoof as many browser values as possible, as doing that in some cases (user agent, OS, screen size) increase entropy (uniquely identifiable information). In this project I aim to research about, evaluate, test configuration sets and compatibility among the extensions and configurations that can help protect our privacy while browsing the web. Suggestions, corrections from all are welcome.


### Disclaimer.<a name="disclaimer"></a>
This project is a collection of configurations to setup firefox' preferences and to setup and install third party extensions/addons.
These extensions have been carefully chosen. They are downloaded straight from the "Firefox Add-ons store". Each addon is fully open source and anyone can view the code. Each of the addon's developer has high reputation and multi thousand downloads in the addons store. Because these addons are not developed by me, use them at your own risk.


## Requirements
Latest stable version of Firefox: 66

Why Firefox?
Chrome and firefox are the 2 browsers driving web technologies. Most other browsers are based on either Chromium or Firefox.
Chrome collects all sorts of data and every single URL you visit. If you no choice (are you sure?) but to use Chrome at least opt-out of the data collection "features" that you can opt-out of (https://support.google.com/chrome/answer/114836). You can't opt out of everything, even chromium has tracking builtin deep in it's architecture.

If you are using Chrome or Edge with default settings, not only they don't provide any privacy protection from third parties on the web.
...
#### One For All, All For One:<a name="allforoneoneforall"></a>
The project aims to minimise uniquely identifiable info about the users (Entropy). Using relatively project called "Tor Uplift" which is bringing Tor browser's fingerprint resisting techniques into Firefox[https://wiki.mozilla.org/Security/Fusion#Project_Goals]. (privacy.resistFingerprinting RFP for short)
There are far too many ways of fingerprinting users (Browser/OS versions, installed fonts, screen size, canvas fingerprint, CSS Media Queries, WebGL fingerprint and hundreds of other Web Browser Features[https://browserleaks.com/features]).[https://github.com/CHEF-KOCH/Online-Privacy-Test-Resource-List]

If you fake or even randomize certain values, you can still be identified as the person who fakes only those specific values.
If you disable certain functionalities that are used in fingerprinting, same again.
**We can't beat first party fingerprinting with individual configuration sets** (or even third party fingerprinting, unless we have them blacklisted).
Instead manually trying to change values such as user agent, installed fonts, etc.
I leave that to the brilliant minds behind the Tor project (using privacy.resistFingerprinting). They have worked/working hard to minimize entropy by deciding which variables should be spoofed and to what values.

**When we minimise the uniquely identifiable information, i.e entropy given out by your browser AND we have enough people using the exact set of configuration, only then we could have online privacy.**
(by disabling certain functionalities and faking certain values)


## Installed/Configured Tools and Their Benefits.

This project focuses on researching about the available privacy tools, evaluating, configuring, testing and packaging them. doesn't create anything new. A huge thanks all the brilliant people behind these tools that have spent so much time and energy into making the world a better place.

0. [Mozilla Firefox](https://www.mozilla.org/en-US/firefox/new/): Developed by the non profit organization Mozilla, driving browser technologies and the only viable competitor of Chrome (by Google). Nothing like this would be possible without Firefox.
...
5. [Firefox Multi-Account Containers](https://addons.mozilla.org/en-US/firefox/addon/multi-account-containers/): Mozilla's revolutionary approach to isolate online identities by containing cookies and local storage in multiple separate containers, allowing you to use the web with multiple identities or accounts simultaneously.
...
Have to maximise window everytime.
Temp containers can't go back when container changes.
Some functionality like, voice/video chat would break.
Please Report the issues found.
...
* The reported time zone is set to UTC by FPR.
...
3. You can file a breakage bug report here.
